...
void getverticalorder(Node* root,map<int,vector<int> > &m,int index)
{
	if(root == NULL) return ;
	m[index].push_back(root->data);
	getverticalorder(root->left,m,index-1);
	getverticalorder(root->right,m,index+1);
}
